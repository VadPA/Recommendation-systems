...
class MainRecommender:
    ...
    def extend_with_top_popular(self, recommendations, N):
        """Если кол-во рекоммендаций < N, то дополняем их топ-популярными"""

        if len(recommendations) < N:
            # recommendations.extend(self.popularity_all[len(recommendations): N + 1])
            recommendations.extend(self.popularity_all[:(N - len(recommendations) + 1)])
            recommendations = recommendations[:N]

        return recommendations

    def get_recommendations(self, user, N, model):
        ...
        res = self.extend_with_top_popular(res, N)
        return res

    def get_als_recommendations(self, user, N):
        """Рекомендации через стардартные библиотеки implicit"""

        return self.get_recommendations(user, N, model=self.model)

    def get_own_recommendations(self, user, N=5):
        """Рекомендуем товары среди тех, которые юзер уже купил"""

        return self.get_recommendations(user, N, model=self.own_recommender)
